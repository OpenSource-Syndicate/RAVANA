...
def test_imports():
    ...
    try:
        ...
        print(
            f"[OK] Physics prompts loaded: {len(ADVANCED_PHYSICS_EXPERIMENTS)} experiments, {len(DISCOVERY_PROMPTS)} discovery prompts")
...
def test_experiment_prompts():
    ...
    try:
        ...
        print(
            f"[OK] Random experiment: {random_exp['name']} ({random_exp['difficulty']})")
...
def run_basic_experiment_test():
    ...
    try:
        ...
        print(
            f"[INFO] Simulation type: {result.get('simulation_type', 'Unknown')}")
        print(
            f"[INFO] Final verdict: {result.get('final_verdict', 'No verdict')[:100]}...")

        if result.get('generated_code'):
            print(
                f"[INFO] Code generated: {len(result['generated_code'])} characters")

        if result.get('execution_result'):
            print(
                f"[INFO] Execution result available: {len(str(result['execution_result']))} characters")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        if sys.argv[1] == "experiments":
            ...
            print(
                "Usage: python test_system_simple.py [system|experiments|experiment]")
    else:
