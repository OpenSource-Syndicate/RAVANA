...
async def demo_blog_publishing():
    ...
    try:
        ...
        print(
            f"  Generation Time: {dry_result['generation_time_seconds']:.2f} seconds")
        print()
        ...
        publish_choice = input(
            "Would you like to publish this to the actual blog? [y/N]: ").lower()
        ...
        if publish_choice == 'y':
            ...
            if publish_result['status'] == 'success':
                print(
                    f"  Published URL: {publish_result.get('published_url', 'N/A')}")
                print(f"  Post ID: {publish_result.get('post_id', 'N/A')}")
                print(
                    f"  Total Time: {publish_result.get('publish_time_seconds', 0):.2f} seconds")
                print("\n🎉 Blog post published successfully!")
            else:
                print(
                    f"  Error: {publish_result.get('message', 'Unknown error')}")
                print(
                    f"  Error Code: {publish_result.get('error', 'UNKNOWN')}")
        ...
        print("  -d '{")
        print(
            f'    "title": "{dry_result.get("title", "Time Dilation Hack")}",')
...
async def demo_various_styles():
    ...
    try:
        ...
        styles = ["technical", "casual",
                  "academic", "creative", "philosophical"]
