...
def test_emotional_intelligence():
    ...
    print(
        f"   Confident mood changed from {initial_confident} to {ei.mood_vector['Confident']}")
    ...
    ei.blend_moods()
    print(
        f"   Inspired mood changed from {previous_inspired} to {ei.mood_vector['Inspired']}")
    ...
    print(
        f"   Emotional events: {initial_events} -> {len(ei.emotional_events)}")
