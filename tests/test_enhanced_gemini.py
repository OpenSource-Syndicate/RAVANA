...
def test_basic_functionality():
    ...
    for i, prompt in enumerate(test_prompts, 1):
        ...
        if result and not result.startswith("[") and not result.startswith("Error"):
            print(
                f"✅ Success: {result[:100]}{'...' if len(result) > 100 else ''}")
        else:
...
def test_rate_limit_simulation():
    ...
    for i in range(5):
        ...
        stats = get_gemini_key_statistics()
        print(
            f"Available keys: {stats['available_keys']}/{stats['total_keys']}")
...
def test_error_handling():
    ...
    for i, prompt in enumerate(problematic_prompts, 1):
        ...
        try:
            ...
            if result and not result.startswith("["):
                print(
                    f"✅ Handled gracefully: {result[:50]}{'...' if len(result) > 50 else ''}")
            else:
...
def test_configuration_loading():
    ...
    try:
        ...
        if stats['total_keys'] >= 10:
            print(
                f"✅ Configuration loaded successfully: {stats['total_keys']} keys found")

            # Verify key priorities
            priorities = [key_data['priority']
                          for key_data in stats['keys'].values()]
