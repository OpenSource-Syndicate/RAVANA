...
'''
        ...
        metrics, issues = self.analyzer.analyze_ast(
            compile(code, '<test>', 'exec'), lines)
        ...
        param_issues = [
            issue for issue in issues if "parameters" in issue.description]
...
'''
        ...
        metrics, issues = self.analyzer.analyze_ast(
            compile(complex_code, '<test>', 'exec'), lines)

        # Should detect high complexity
        self.assertTrue(metrics.complexity > 5)
        complexity_issues = [
            issue for issue in issues if "complexity" in issue.description]
...
'''
        ...
        secret_issues = [
            issue for issue in issues if "secret" in issue.description.lower()]
...
'''
        ...
        sync_issues = [
            issue for issue in issues if "synchronous" in issue.description.lower()]
...
class SnakeAgentIntegrationTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.mock_coding_llm.analyze_code = AsyncMock(
            return_value="Code analysis result")
        self.mock_coding_llm.generate_improvement = AsyncMock(
            return_value="Improved code")
        self.mock_reasoning_llm.evaluate_safety = AsyncMock(
            return_value={"risk_level": "LOW"})
        self.mock_reasoning_llm.plan_communication = AsyncMock(
            return_value={"channels": ["logging"]})
    ...
    async def test_snake_agent_initialization(self):
        ...
        with patch('core.snake_agent.SnakeConfigValidator.get_startup_report') as mock_report:
            mock_report.return_value = {
                "config_valid": True, "ollama_connected": True, "available_models": []}
            ...
            with patch('core.snake_agent.SnakeConfigValidator.get_startup_report') as mock_report:
                ...
                with patch('core.snake_agent.create_snake_coding_llm') as mock_coding:
                    with patch('core.snake_agent.create_snake_reasoning_llm') as mock_reasoning:
                        ...
                        self.assertTrue(
                            len(snake_agent.state.pending_experiments) > 0)
...
class SnakeAgentValidationSuite:
    ...
    async def validate_configuration(self) -> Dict[str, Any]:
        ...
        for model_name, model_config in [("coding", coding_model), ("reasoning", reasoning_model)]:
            ...
            if missing_keys:
                raise ValueError(
                    f"Missing keys in {model_name} model config: {missing_keys}")
    ...
    async def validate_code_analysis(self) -> Dict[str, Any]:
        ...
        pattern_issues = pattern_analyzer.analyze_patterns(
            test_code, "test.py")
    ...
    async def validate_sandbox_safety(self) -> Dict[str, Any]:
        ...
        is_unsafe, warnings, score = safety_analyzer.analyze_safety(
            dangerous_code)
