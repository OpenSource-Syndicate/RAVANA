...
async def main():
    ...
    try:
        ...
        print(
            f"   • Graceful shutdown: {'enabled' if Config.GRACEFUL_SHUTDOWN_ENABLED else 'disabled'}")
        print(
            f"   • State persistence: {'enabled' if Config.STATE_PERSISTENCE_ENABLED else 'disabled'}")
        print(f"   • Shutdown timeout: {Config.SHUTDOWN_TIMEOUT}s")
        print(f"   • Force timeout: {Config.FORCE_SHUTDOWN_AFTER}s")
        print(
            f"   • Memory service timeout: {Config.MEMORY_SERVICE_SHUTDOWN_TIMEOUT}s")
