...
class PhysicsExperimentTester:
    ...
    async def test_single_experiment(self, experiment_data):
        ...
        try:
            ...
            result['execution_result'] = experiment_result.get(
                'execution_result')
            result['online_validation'] = experiment_result.get(
                'online_validation')
    ...
    async def test_discovery_mode(self, discovery_prompt):
        ...
        logger.info(
            f"Testing discovery mode with prompt: {discovery_prompt[:100]}...")
        ...
        try:
            ...
            result['success'] = True
            result['generated_response'] = experiment_result.get(
                'refined_idea')
            result['simulation_approach'] = experiment_result.get(
                'simulation_type')
            ...
            if experiment_result.get('refined_idea'):
                ...
                creative_words = ['novel', 'innovative',
                                  'unprecedented', 'breakthrough', 'revolutionary']
                result['creativity_score'] = sum(
                    1 for word in creative_words if word in idea_text)

                # Scientific plausibility indicators
                science_words = ['theory', 'equation',
                                 'measurement', 'experiment', 'hypothesis']
                result['scientific_plausibility'] = sum(
                    1 for word in science_words if word in idea_text)
    ...
    async def test_agi_integration(self, experiment_prompt):
        ...
        try:
            ...
            try:
                ...
                stmt = select(ExperimentLog).order_by(
                    ExperimentLog.timestamp.desc()).limit(5)
    ...
    def generate_report(self):
        ...
        avg_execution_time = sum(r['execution_time']
                                 for r in self.results) / total_experiments
    ...
    try:
        ...
        for difficulty in ['intermediate', 'advanced', 'expert']:
            ...
            if experiments:
                ...
                logger.info(
                    f"Testing {difficulty} experiment: {experiment['name']}")
                result = await tester.test_single_experiment(experiment)

                if result['success']:
                    logger.info(
                        f"✓ {experiment['name']} completed successfully")
                else:
                    logger.error(
                        f"✗ {experiment['name']} failed: {result.get('error', 'Unknown error')}")

        # Test 2: Discovery mode tests
        logger.info("=== PHASE 2: TESTING DISCOVERY MODE ===")

        # Test first 3
        for i, discovery_prompt in enumerate(DISCOVERY_PROMPTS[:3], 1):
            ...
            if discovery_result['success']:
                ...
                logger.info(
                    f"  Creativity Score: {discovery_result['creativity_score']}")
                logger.info(
                    f"  Scientific Plausibility: {discovery_result['scientific_plausibility']}")
            else:
        ...
        if integration_result['integration_success']:
            ...
            logger.error(
                f"✗ AGI system integration test failed: {integration_result.get('error')}")
...
async def run_single_experiment_test(experiment_name=None):
    ...
    try:
        ...
        if experiment_name:
            ...
            experiment = next(
                (exp for exp in ADVANCED_PHYSICS_EXPERIMENTS if exp['name'] == experiment_name), None)
            if not experiment:
        ...
        if result['success']:
            print(
                f"SCIENTIFIC VALIDITY: {result.get('scientific_validity', 'Unknown')}")
