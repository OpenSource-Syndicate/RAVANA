...
try:
    from core.vltm_data_models import (
        MemoryType, MemoryImportance, PatternType, VLTMConfiguration,
...
class TestPerformanceMonitor:
    ...
    async def test_operation_tracking(self):
        ...
        # Start operation
        op_id = monitor.start_operation(OperationType.MEMORY_STORE, metadata={"test": "data"})
        assert op_id != ""
...
class TestVLTMIntegration:
    ...
    async def test_end_to_end_memory_lifecycle(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            vltm_store = VeryLongTermMemoryStore(storage_backend=mock_storage_backend)
