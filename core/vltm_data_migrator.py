...
class VLTMDataMigrator:
    ...
    async def migrate_all_data(self) -> MigrationStats:
        ...
        try:
            ...
            logger.info(
                f"Migration completed: {stats.episodic_migrated + stats.knowledge_migrated} memories migrated")
            return stats
    ...
    async def _migrate_knowledge_data(self) -> Dict[str, int]:
        ...
        try:
            ...
            for category in categories:
                try:
                    category_data = self.knowledge_service.get_knowledge_by_category(
                        category, limit=20)
                    knowledge_entries.extend(category_data)
                except Exception as e:
                    logger.warning(
                        f"Error fetching knowledge for {category}: {e}")

            for knowledge_data in knowledge_entries:
                try:
                    vltm_memory = self._convert_knowledge_to_vltm(
                        knowledge_data)
                    if not vltm_memory:
                        continue
    ...
    async def _is_duplicate(self, vltm_memory: Dict[str, Any]) -> bool:
        ...
        try:
            content = vltm_memory["content"]
            search_text = (content.get("original_text")
                           or content.get("knowledge_summary", ""))[:100]
