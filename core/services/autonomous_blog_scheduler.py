...
class AutonomousBlogScheduler:
    ...
    def __init__(self, agi_system=None):
        ...
        self.enabled = Config.BLOG_ENABLED and getattr(Config, 'BLOG_AUTO_PUBLISH_ENABLED', False)
        self.min_post_interval_hours = getattr(Config, 'BLOG_PUBLISH_FREQUENCY_HOURS', 24)
        self.max_pending_events = getattr(Config, 'BLOG_MAX_PENDING_EVENTS', 10)
        self.min_importance_threshold = getattr(Config, 'BLOG_MIN_IMPORTANCE_THRESHOLD', 0.6)
        
        logger.info(f"AutonomousBlogScheduler initialized: enabled={self.enabled}, learning_generator={'available' if self.learning_generator else 'not available'}")
    ...
    async def register_learning_event(
        ...
        """
        if not self.enabled:
            logger.debug("Autonomous blogging disabled, skipping event registration")
            return False
        
        if importance_score < self.min_importance_threshold:
            logger.debug(f"Event importance {importance_score} below threshold {self.min_importance_threshold}")
            return False
        ...
        if len(self.pending_events) > self.max_pending_events:
            ...
            self.pending_events.sort(key=lambda e: e.importance_score, reverse=True)
            self.pending_events = self.pending_events[:self.max_pending_events]
        
        logger.info(f"Registered learning event: {trigger_type.value} - {topic}")
    ...
    async def _should_trigger_blog_post(self, event: BlogTriggerEvent) -> bool:
        ...
        if event.importance_score >= 0.9:
            logger.info(f"High importance event ({event.importance_score}) triggering blog post")
            return True
        ...
        if event.emotional_valence < -0.5 and event.importance_score >= 0.7:
            logger.info(f"Significant failure event triggering blog post for learning")
            return True
        ...
        recent_events = [e for e in self.pending_events 
                        if (datetime.now() - e.timestamp).total_seconds() < (6 * 3600)]  # Last 6 hours
        
        if len(recent_events) >= 3 and sum(e.importance_score for e in recent_events) >= 2.0:
            logger.info(f"Multiple learning events triggering consolidated blog post")
            return True
    ...
    async def _trigger_autonomous_blog_post(self, primary_event: BlogTriggerEvent) -> bool:
        ...
        try:
            ...
            blog_action = BlogPublishAction(self.agi_system, self.agi_system.data_service)
            ...
            if result.get('status') == 'success':
                ...
                logger.info(f"Successfully published autonomous blog post: {result.get('title')}")
                return True
            else:
                logger.warning(f"Failed to publish autonomous blog post: {result.get('message')}")
                return False
    ...
    def clear_old_events(self, hours: int = 48):
        ...
        self.pending_events = [e for e in self.pending_events if e.timestamp > cutoff]
