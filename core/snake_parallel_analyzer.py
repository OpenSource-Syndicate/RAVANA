...
class CodeQualityAnalyzer:
    ...
    def _calculate_maintainability(self, code_content: str, complexity: float) -> float:
        ...
        maintainability = max(
            0, (171 - 5.2 * complexity - 0.23 * volume) * 100 / 171)
    ...
    def _detect_performance_issues(self, code_content: str, tree: ast.AST) -> List[str]:
        ...
        perf_patterns = [
            (r'\.append\s*\(.*\)\s*in.*for.*in',
             "List comprehension may be faster than append in loop"),
            (r'range\s*\(\s*len\s*\(',
             "Consider using enumerate() instead of range(len())"),
            (r'\.keys\s*\(\s*\).*in',
             "Direct dictionary iteration is faster than .keys()"),
        ]
    ...
    def _detect_style_violations(self, code_content: str) -> List[str]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if len(line) > 88:
                violations.append(
                    f"Line {i}: Line too long ({len(line)} > 88)")
    ...
    def _process_analysis_task(self, task: AnalysisTask, analyzer: CodeQualityAnalyzer) -> Optional[AnalysisResult]:
        ...
        try:
            ...
            if task.file_path.endswith('.py'):
                metrics = analyzer.analyze_python_code(
                    code_content, task.file_path)
            else:
