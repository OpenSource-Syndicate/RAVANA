...
class VeryLongTermMemoryStore:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if config_issues:
                logger.error(f"Configuration validation failed: {config_issues}")
                return False
            
            # Initialize storage backend
            self.storage_backend = StorageBackend(self.config, self.base_storage_dir)
            ...
            logger.error(f"Failed to initialize VLTM store: {e}", exc_info=True)
            return False
    ...
    async def store_memory(
        ...
        try:
            ...
            if memory_type is None:
                classification_result = self.memory_classifier.classify_memory(memory_record)
            ...
            if use_semantic_search:
                ...
                for semantic_result in semantic_results:
                    ...
                    if memory_data:
                        ...
                        # Add search metadata
                        memory_data["search_score"] = 1.0 - semantic_result["distance"]
                    ...
                    for memory_type in memory_types:
                        ...
                        for db_memory in db_results:
                            if db_memory.strategic_value >= min_strategic_value:
                                ...
                                memory_data["search_score"] = 0.5  # Default score for DB results
                                results.append(memory_data)
    ...
    async def store_strategic_knowledge(
        ...
        try:
            ...
            if success:
                ...
                logger.error(f"Failed to store strategic knowledge {knowledge_id}")
                return None
                
        except Exception as e:
            logger.error(f"Error storing strategic knowledge: {e}", exc_info=True)
            return None
    ...
    async def query_strategic_knowledge(self, query: StrategicQuery) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results.sort(key=lambda x: x.get("confidence_level", 0), reverse=True)
    ...
    async def _load_statistics(self):
        ...
        try:
            ...
            self.total_patterns_extracted = storage_stats.get("pattern_count", 0)
            self.total_strategic_knowledge = storage_stats.get("strategic_knowledge_count", 0)
