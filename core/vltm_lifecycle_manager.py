...
class MemoryLifecycleManager:
    ...
    async def _evaluate_memory_promotion(self, memory: VeryLongTermMemory) -> Optional[LifecycleAction]:
        ...
        try:
            ...
            logger.error(f"Error evaluating promotion for memory {memory.memory_id}: {e}")
            return None
    ...
    async def _execute_promotion_action(self, action: LifecycleAction) -> bool:
        ...
        try:
            ...
            logger.info(f"Promoting memory {action.memory_id} from {action.from_stage} to {action.to_stage}")
            ...
            logger.error(f"Error executing promotion action {action.action_id}: {e}")
            return False
    ...
    async def _execute_archival_action(self, action: LifecycleAction, policy: ArchivalPolicy) -> bool:
        ...
        try:
            ...
            logger.info(f"Archiving memory {action.memory_id} with {policy.compression_level} compression")
            ...
            logger.error(f"Error executing archival action {action.action_id}: {e}")
            return False
    ...
    async def _execute_deletion_action(self, action: LifecycleAction) -> bool:
        ...
        try:
            ...
            logger.error(f"Error executing deletion action {action.action_id}: {e}")
            return False
    ...
    async def promote_memory_manually(self, memory_id: str, to_stage: MemoryLifecycleStage, reason: str) -> bool:
        ...
        try:
            ...
            if not memory:
                logger.warning(f"Memory {memory_id} not found for manual promotion")
                return False
            ...
            if success:
                self.lifecycle_actions.append(action)
                logger.info(f"Manually promoted memory {memory_id} to {to_stage}")
            
            return success
            
        except Exception as e:
            logger.error(f"Error in manual promotion of memory {memory_id}: {e}")
            return False
