...
class SnakeFileEventHandler(FileSystemEventHandler):
    ...
    def on_moved(self, event):
        if not event.is_directory:
            self.file_monitor._handle_file_event("moved", event.dest_path, event.src_path)
...
class ContinuousFileMonitor:
    ...
    def __init__(self, snake_agent, config: SnakeAgentConfiguration, log_manager: SnakeLogManager):
        ...
        self.monitored_extensions = {'.py', '.json', '.md', '.txt', '.yml', '.yaml', '.toml'}
        self.excluded_dirs = {'__pycache__', '.git', '.venv', 'node_modules', '.qoder', 'snake_logs'}
    ...
    def _periodic_scan(self):
        ...
        try:
            ...
            for relative_path in deleted_files:
                self._handle_file_event("deleted", str(self.root_path / relative_path))
                changes_found += 1
    ...
    def _log_info(self, event_type: str, data: Dict):
        ...
        try:
            ...
            loop.run_until_complete(
                self.log_manager.log_system_event(event_type, data, worker_id="file_monitor")
            )
            loop.close()
        except Exception:
    ...
    def _log_error(self, event_type: str, data: Dict):
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            loop.run_until_complete(
                self.log_manager.log_system_event(event_type, data, level="error", worker_id="file_monitor")
            )
            loop.close()
        except Exception:
            pass  # Ignore logging errors in worker thread
