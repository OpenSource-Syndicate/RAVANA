...
class MultiModalIndex:
    ...
    async def _build_initial_indices(self):
        ...
        try:
            ...
            logger.info(
                f"Building initial indices for {len(memories)} memories")
    ...
    async def index_memory(self, memory: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error indexing memory {memory.get('memory_id')}: {e}")
            return False
    ...
    async def multi_dimensional_query(self,
                                      temporal_range: Optional[Tuple[datetime,
                                                                     datetime]] = None,
        ...
        try:
            ...
            if temporal_range:
                ...
                candidates.update(m.get("memory_id")
                                  for m in temporal_results if m.get("memory_id"))

            # Semantic candidates
            if semantic_query:
                semantic_results = await self.query_semantic(semantic_query, limit=limit * 2)
                semantic_ids = set(m.get("memory_id")
                                   for m in semantic_results if m.get("memory_id"))
            ...
            strategic_ids = set(m.get("memory_id")
                                for m in strategic_results if m.get("memory_id"))
            ...
            final_results.sort(key=lambda m: m.get(
    ...
    async def rebuild_index(self, index_type: IndexType) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Rebuilt {index_type} index with {self.index_stats[index_type].total_entries} entries")
            return True
    ...
    async def rebuild_all_indices(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Rebuilt {success_count}/{len(IndexType)} indices successfully")
    ...
    def get_index_statistics(self) -> Dict[str, Any]:
        ...
        total_entries = sum(
            stats.total_entries for stats in self.index_stats.values())
...
class TemporalIndexer:
    ...
    async def index_memory(self, memory: Dict[str, Any], temporal_index: Dict[str, Any]):
        ...
        try:
            ...
            time_key = created_at.strftime(
                "%Y-%m") if isinstance(created_at, datetime) else str(created_at)[:7]
...
class CausalIndexer:
    ...
    async def index_memory(self, memory: Dict[str, Any], causal_index: Dict[str, Any]):
        ...
        try:
            ...
            causal_indicators = ["caused", "led to",
                                 "resulted in", "because", "due to"]
...
class StrategicIndexer:
    ...
    async def query(self, strategic_index: Dict[str, Any], strategic_value_threshold: float,
        ...
        try:
            ...
            target_types = [str(mt) for mt in memory_types] if memory_types else list(
                strategic_index.keys())
            ...
            results.sort(key=lambda x: x.get(
                "strategic_value", 0), reverse=True)
            return results[:limit]
