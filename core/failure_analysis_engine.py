...
class FailureAnalysisEngine:
    ...
    def __init__(self, agi_system, knowledge_service=None):
        ...
        self.mitigation_effectiveness: Dict[str,
    ...
    async def analyze_failure(
        ...
        """
        logger.info(
            f"Starting failure analysis for build: {getattr(build_context, 'build_id', 'unknown')}")
        ...
        logger.info(
            f"Failure analysis complete. Confidence: {confidence_score:.2f}")
    ...
    async def generate_failure_recovery_plan(
        ...
        for recommendation in failure_analysis.improvement_recommendations:
            ...
            if long_term_improvement:
                recovery_plan["long_term_improvements"].append(
        ...
        recovery_plan["resource_requirements"] = self._estimate_recovery_resources(
            recovery_plan)

        # Estimate timeline
        recovery_plan["timeline_estimate"] = self._estimate_recovery_timeline(
            recovery_plan)
    ...
    async def _extract_failure_points(
        ...
        if error_message:
            failure_point = self._analyze_error_message(
                error_message, build_context)
            if failure_point:
        ...
        for result in execution_results:
            if hasattr(result, 'error_message') and result.error_message:
                failure_point = self._analyze_execution_error(
        ...
        for strategy_failure in strategy_failures:
            failure_point = self._analyze_strategy_failure(
                strategy_failure, build_context)
            if failure_point:
                failure_points.append(failure_point)
        ...
        if failure_details.get('timeout', False):
            failure_point = FailurePoint(
                ...
                timestamp=time.time(),
                context={"timeout_duration": failure_details.get(
    ...
    async def _perform_root_cause_analysis(
        ...
        ranked_root_causes = self._rank_root_causes_by_probability(
            unique_root_causes, failure_points)
    ...
    async def _find_similar_failures(
        ...
        failure_signature = self._create_failure_signature(
            failure_details, build_context)
        ...
        for historical_failure in self.failure_history:
            similarity = self._calculate_failure_similarity(
                failure_signature,
                self._create_failure_signature_from_analysis(
                    historical_failure)
            )
    ...
    async def _extract_lessons_learned(
        ...
        prioritized_lessons = self._prioritize_lessons(
            unique_lessons, failure_points)
    ...
    async def _store_failure_analysis(self, analysis_result: FailureAnalysisResult):
        ...
        if self.knowledge_service:
            try:
                ...
                logger.warning(
                    f"Failed to store failure analysis in knowledge base: {e}")
