...
class ParallelExecution:
    ...
    # "best_result", "majority_vote", "ensemble"
    result_aggregation: str = "best_result"
...
class HybridExecution:
    ...
    parallel_phases: List[str] = field(
        default_factory=lambda: ["exploration", "refinement"])
    sequential_phases: List[str] = field(
        default_factory=lambda: ["analysis", "validation"])
...
class MultiStrategyExecutor:
    ...
    async def execute_strategies(
        ...
        **kwargs
        ...
        logger.info(
            f"Executing {len(strategies)} strategies for build {build_id} in {mode.value} mode")
        ...
        try:
            ...
            if mode == ExecutionMode.PARALLEL:
                result = await self._execute_parallel(
                    ...
                    ParallelExecution(
                        max_concurrent=min(
                            len(strategies), self.default_parallel_config.max_concurrent),
            ...
            self.active_executions[execution_id]['results'] = result.get(
                'execution_results', [])
            ...
            logger.error(
                f"Strategy execution failed for build {build_id}: {e}", exc_info=True)
    ...
    async def cancel_execution(self, build_id: str) -> bool:
        ...
        for execution_id, execution in list(self.active_executions.items()):
            if execution['build_id'] == build_id:
                ...
                logger.info(
                    f"Cancelled execution {execution_id} for build {build_id}")
    ...
    async def _execute_parallel(
        ...
        logger.info(
            f"Starting parallel execution of {len(strategies)} strategies")
                ...
                try:
                    ...
                    if result.success:
                        ...
                        if len(successful_results) >= len(strategies) * (1 - config.failure_tolerance):
                            logger.info(
                                f"Early termination: {len(successful_results)} successes achieved")
                            break
    ...
    async def _execute_sequential(
        ...
        logger.info(
            f"Starting sequential execution of {len(strategies)} strategies")
        ...
        for i, strategy in enumerate(strategies):
            logger.info(
                f"Executing strategy {i+1}/{len(strategies)}: {strategy.get('name', 'Unknown')}")

            # Apply accumulated knowledge to strategy
            if config.learning_enabled and accumulated_knowledge:
                strategy = self._adapt_strategy_with_knowledge(
                    strategy, accumulated_knowledge)
            ...
            if config.early_termination and result.success:
                if result.confidence_score >= config.success_threshold:
                    logger.info(
                        f"Early termination: success threshold {config.success_threshold} achieved")
                    break
    ...
    async def _execute_hybrid(
        ...
        logger.info(
            f"Starting hybrid execution of {len(strategies)} strategies")

        all_results = []

        # Phase 1: Parallel exploration of different approaches
        exploration_strategies = self._select_strategies_for_phase(
            strategies, "exploration")
        if exploration_strategies:
            parallel_config = ParallelExecution(
                max_concurrent=min(3, len(exploration_strategies)))
            exploration_results = await self._execute_parallel(context, exploration_strategies, parallel_config)
            all_results.extend(
                exploration_results.get('execution_results', []))
        ...
        if promising_strategies:
            sequential_config = SequentialExecution(
                learning_enabled=True, early_termination=True)
    ...
    async def _execute_adaptive(
        ...
        **kwargs
        ...
        logger.info(
            f"Starting adaptive execution of {len(strategies)} strategies")
        ...
        while iteration < max_iterations and remaining_strategies and not context.cancellation_token.is_set():
            ...
            logger.info(
                f"Adaptive iteration {iteration}/{max_iterations}, mode: {current_mode.value}")
            ...
            if switch_trigger:
                new_mode = self._select_adaptive_mode(
                    current_mode, switch_trigger, all_results)
                if new_mode != current_mode:
                    logger.info(
                        f"Adaptive switch: {current_mode.value} -> {new_mode.value} (trigger: {switch_trigger.value})")
            ...
            if current_mode == ExecutionMode.PARALLEL:
                ...
                config = ParallelExecution(
                    ...
                    timeout_per_strategy=(
                        timeout_seconds - elapsed_time) / batch_size
                )
            ...
            # Collect results
            batch_execution_results = batch_results.get(
                'execution_results', [])
            ...
            if len(successful_results) >= len(strategies) * (1 - failure_tolerance):
                logger.info(
                    f"Adaptive execution terminating: sufficient successes achieved")
                break

            # Check timeout
            if elapsed_time >= timeout_seconds * 0.9:
                logger.info(
                    f"Adaptive execution terminating: timeout approaching")
                break
    ...
    async def _execute_single_strategy_async(
        ...
        try:
            ...
            async with asyncio.timeout(timeout):
                ...
                # Evaluate success
                success, confidence = self._evaluate_strategy_success(
                    execution_artifact, strategy)

                # Extract lessons learned
                lessons = self._extract_lessons_from_execution(
                    strategy, execution_artifact, success)
                ...
                result = ExecutionResult(
                    ...
                    success_factors=strategy.get(
                        'success_factors', []) if success else [],
                    failure_points=strategy.get(
                        'failure_modes', []) if not success else []
                )
            ...
            logger.warning(
                f"Strategy {strategy_name} timed out after {timeout} seconds")
            ...
            logger.error(
                f"Strategy {strategy_name} failed with exception: {e}", exc_info=True)
            return ExecutionResult(
                strategy_id=strategy_id,
                status=ExecutionStatus.FAILED,
                success=False,
    ...
    async def _execute_strategy_steps(
        ...
        for i, step in enumerate(approach_steps):
            logger.info(
                f"Executing step {i+1}/{len(approach_steps)}: {step[:50]}...")
        ...
        final_artifact = self._combine_intermediate_results(
            intermediate_results, strategy)
