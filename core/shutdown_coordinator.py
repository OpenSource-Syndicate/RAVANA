...
class ShutdownCoordinator:
    ...
    def register_component(self, component: Any, priority: ShutdownPriority, is_async: bool = True):
        ...
        logger.debug(
            f"Registered component with priority {priority.name}: {type(component).__name__}")
    ...
    def register_cleanup_handler(self, handler: Callable, is_async: bool = False):
        ...
        logger.debug(
            f"Registered {'async' if is_async else 'sync'} cleanup handler: {handler.__name__}")
    ...
    async def initiate_shutdown(self, reason: str = "manual"):
        ...
        try:
            ...
            logger.error(
                f"❌ Error during graceful shutdown: {e}", exc_info=True)
    ...
    async def _execute_shutdown_phases(self):
        ...
        self.shutdown_state["completed_phases"].append(
    ...
    async def _execute_phase(self, phase_name: ShutdownPhase, phase_handler: Callable):
        ...
        try:
            ...
            logger.info(
                f"✅ Phase '{phase_name.value}' completed in {phase_duration:.2f}s")

            self.shutdown_state["completed_phases"].append(phase_name.value)

        except Exception as e:
            logger.error(
                f"❌ Error in shutdown phase '{phase_name.value}': {e}", exc_info=True)
            self.shutdown_state["errors"].append(
                f"{phase_name.value}: {str(e)}")
        ...
        for registration in self.registered_components:
            try:
                ...
                if hasattr(component, 'get_health_status'):
                    ...
                    logger.info(
                        f"Component {type(component).__name__} health: {health}")
            except Exception as e:
                logger.warning(
                    f"Health check failed for {type(component).__name__}: {e}")
    ...
    async def _phase_component_notification(self):
        ...
        logger.info(
            f"Notifying {len(self.registered_components)} registered components of shutdown...")
                    ...
                    try:
                        if registration.is_async:
                            await asyncio.wait_for(
                                ...
                                timeout=getattr(
                                    Config, 'COMPONENT_PREPARE_TIMEOUT', 10.0)
                            )
                        else:
                            component.prepare_shutdown()
                        logger.debug(
                            f"Component {type(component).__name__} prepared for shutdown")
                    except Exception as e:
                        logger.warning(
                            f"Error in prepare_shutdown for {type(component).__name__}: {e}")
                else:
                    logger.debug(
                        f"Component {type(component).__name__} has no prepare_shutdown method")
            except asyncio.TimeoutError:
                logger.warning(
                    f"Component {type(component).__name__} prepare_shutdown timed out")
            except Exception as e:
                logger.error(
                    f"Error preparing component {type(component).__name__} for shutdown: {e}")
    ...
    async def _phase_stop_background_tasks(self):
        ...
        logger.info(
            f"Cancelling {len(self.agi_system.background_tasks)} background tasks...")
        ...
        if tasks_to_cancel:
            try:
                ...
                completed_tasks = [
                    task for task in tasks_to_cancel if task.done()]
                cancelled_tasks = [
                    task for task in tasks_to_cancel if task.cancelled()]
                logger.info(
                    f"Background tasks handled: {len(completed_tasks)} completed, {len(cancelled_tasks)} cancelled")
            except asyncio.TimeoutError:
                logger.warning(
                    "Timeout while waiting for background tasks to cancel")
                # Check for any remaining tasks that didn't finish
                remaining_tasks = [
                    task for task in tasks_to_cancel if not task.done() and not task.cancelled()]
                if remaining_tasks:
                    logger.info(
                        f"Cancelling {len(remaining_tasks)} remaining tasks...")
                    for task in remaining_tasks:
                        if not task.done():
                            task.cancel()
                    # Give them a brief moment to process cancellation
                    try:
                        await asyncio.wait_for(
                            asyncio.gather(*remaining_tasks,
                                           return_exceptions=True),
                            timeout=2.0  # Short timeout to handle remaining cancellations
                        )
                    except asyncio.TimeoutError:
                        logger.warning(
    ...
    async def _phase_resource_cleanup(self):
        ...
        for handler in self.cleanup_handlers:
            try:
                handler()
                logger.debug(
                    f"Executed sync cleanup handler: {handler.__name__}")
            except Exception as e:
                logger.error(
                    f"Error in sync cleanup handler {handler.__name__}: {e}")
        ...
        for handler in self.async_cleanup_handlers:
            try:
                ...
                logger.warning(
                    f"Async cleanup handler {handler.__name__} exceeded timeout")
            except Exception as e:
                logger.error(
                    f"Error in async cleanup handler {handler.__name__}: {e}")
    ...
    async def _phase_service_shutdown(self):
        ...
        component_shutdown_timeout = getattr(
            Config, 'COMPONENT_SHUTDOWN_TIMEOUT', 15.0)
        ...
        for registration in self.registered_components:
            try:
                ...
                if hasattr(component, 'shutdown'):
                    logger.info(
                        f"Shutting down component: {type(component).__name__}")
                    try:
                        if registration.is_async:
                        ...
                        if success:
                            logger.info(
                                f"Component {type(component).__name__} shutdown successful")
                        else:
                            logger.warning(
                                f"Component {type(component).__name__} shutdown reported failure")
                    except Exception as e:
                        logger.warning(
                            f"Error during shutdown of {type(component).__name__}: {e}")
                        success = False
                    ...
                    if hasattr(component, 'get_shutdown_metrics'):
                        try:
                            ...
                            self.shutdown_state["component_metrics"][type(
                                component).__name__] = metrics
                        except Exception as e:
                            logger.warning(
                                f"Error collecting metrics for {type(component).__name__}: {e}")
                else:
                    logger.debug(
                        f"Component {type(component).__name__} has no shutdown method")
            except asyncio.TimeoutError:
                logger.warning(
                    f"Component {type(component).__name__} shutdown timed out")
            except Exception as e:
                logger.error(
    ...
    async def _phase_final_validation(self):
        ...
        elapsed = (datetime.utcnow() -
                   self.shutdown_start_time).total_seconds()
        ...
        try:
            ...
            for field in required_fields:
                if field not in state_data:
                    logger.warning(
                        f"Missing required field in state data: {field}")
                    return False
    ...
    async def _collect_system_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.agi_system, 'current_plan'):
                ...
                agi_state["current_task_prompt"] = getattr(
                    self.agi_system, 'current_task_prompt', None)
            ...
            if hasattr(self.agi_system, 'research_in_progress'):
                agi_state["research_in_progress"] = list(
                    self.agi_system.research_in_progress.keys())

            # Invention history
            if hasattr(self.agi_system, 'invention_history'):
                # Last 10
                agi_state["invention_history"] = self.agi_system.invention_history[-10:]
    ...
    def _log_shutdown_summary(self):
        ...
        logger.info(
            f"Completed Phases: {len(self.shutdown_state['completed_phases'])}")
