...
class BlogContentValidator:
    ...
    def validate_and_sanitize(
        ...
        try:
            ...
            sanitized_title = self._validate_and_sanitize_title(
                title, validation_report)

            # Validate and sanitize content
            sanitized_content = self._validate_and_sanitize_content(
                content, validation_report)

            # Validate and sanitize tags
            sanitized_tags = self._validate_and_sanitize_tags(
                tags, validation_report)

            # Calculate quality scores
            validation_report["quality_score"] = self._calculate_quality_score(
                sanitized_content)
            validation_report["readability_score"] = self._calculate_readability_score(
                sanitized_content)
    ...
    def _validate_and_sanitize_title(self, title: str, report: Dict[str, Any]) -> str:
        ...
        if not title or not title.strip():
            raise ContentValidationError(
                "Title cannot be empty", "EMPTY_TITLE")

        # Sanitize title
        sanitized_title = title.strip()

        # Remove potentially harmful content
        sanitized_title = self._remove_security_threats(
            sanitized_title, "title")

        # Length validation
        if len(sanitized_title) < 5:
            raise ContentValidationError(
                "Title is too short (minimum 5 characters)", "TITLE_TOO_SHORT")
        ...
        if not re.search(r'[A-Za-z]', sanitized_title):
            report["warnings"].append(
                "Title contains no alphabetic characters")
    ...
    def _validate_and_sanitize_content(self, content: str, report: Dict[str, Any]) -> str:
        ...
        if not content or not content.strip():
            raise ContentValidationError(
                "Content cannot be empty", "EMPTY_CONTENT")

        sanitized_content = content.strip()

        # Remove security threats
        sanitized_content = self._remove_security_threats(
    ...
    def _validate_and_sanitize_tags(self, tags: List[str], report: Dict[str, Any]) -> List[str]:
        ...
        for tag in tags[:self.max_tags]:  # Limit number of tags
            ...
            if sanitized_tag and len(sanitized_tag) >= 2:
                ...
                report["warnings"].append(
                    f"Tag '{tag}' removed (invalid or too short)")

        if len(tags) > self.max_tags:
            report["fixes_applied"].append(
                f"Reduced tags from {len(tags)} to {self.max_tags}")
    ...
    def _validate_markdown_structure(self, content: str, report: Dict[str, Any]) -> None:
        ...
        if unmatched_code:
            issues.append(
                "Unmatched backticks found - may cause formatting issues")

        # Check for very long lines (readability)
        lines = content.split('\n')
        long_lines = [i for i, line in enumerate(lines) if len(
            line) > 120 and not line.startswith('#')]
        if len(long_lines) > len(lines) * 0.3:  # More than 30% of lines are too long
            issues.append(
                "Many lines exceed 120 characters - may affect readability")
    ...
    def _check_content_quality(self, content: str, report: Dict[str, Any]) -> None:
        ...
        if len(short_paragraphs) > len(paragraphs) * 0.5:
            issues.append(
                "Many paragraphs are very short - may lack substance")
    ...
    def _perform_final_validation(
        ...
        if len(set(sentences)) < len(sentences) * 0.8:  # More than 20% duplicate sentences
            report["warnings"].append(
                "Content contains many similar sentences")
