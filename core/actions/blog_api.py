...
class BlogAPIInterface:
    ...
    async def publish_post(self, title: str, content: str, tags: List[str]) -> Dict[str, Any]:
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                logger.info(
                    f"Blog post published successfully: {result.get('published_url', 'URL not provided')}")
                ...
                if e.error_type == "RATE_LIMITED" and e.retry_after:
                    ...
                    if attempt == self.retry_attempts - 1:
                        raise
                    logger.warning(
                        f"Rate limited, waiting {e.retry_after} seconds...")
                    await asyncio.sleep(e.retry_after)
                    continue

                if attempt == self.retry_attempts - 1:
                    logger.error(
                        f"Failed to publish blog post after {self.retry_attempts} attempts: {e}")
                    raise
    ...
    async def _make_request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        try:
                            ...
                            raise BlogAPIError(
                                f"Invalid JSON response: {e}", "INVALID_RESPONSE")

                    elif response.status == 401:
                        raise BlogAPIError(
                            "Authentication failed - check auth token", "AUTHENTICATION_FAILED")

                    elif response.status == 400:
                        raise BlogAPIError(
                            f"Invalid request: {response_text}", "INVALID_REQUEST")
                        ...
                        raise BlogAPIError(
                            f"Server error ({response.status}): {response_text}", "SERVER_ERROR")

                    else:
                        raise BlogAPIError(
                            f"HTTP {response.status}: {response_text}", "HTTP_ERROR")
            ...
            raise BlogAPIError(
                f"Request timeout after {self.timeout} seconds", "TIMEOUT")
    ...
    def _handle_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(response, dict):
            raise BlogAPIError(
                "Invalid response format - expected JSON object", "INVALID_RESPONSE")
