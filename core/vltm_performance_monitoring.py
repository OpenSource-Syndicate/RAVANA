...
class PerformanceMonitor:
    ...
    async def start_monitoring(self) -> bool:
        ...
        try:
            ...
            for op_type in OperationType:
                self.operation_profiles[op_type] = OperationProfile(
    ...
    def end_operation(self, operation_id: str,
        ...
        try:
            ...
            execution_time_ms = (
                time.time() - operation_data["start_time"]) * 1000
    ...
    async def record_consolidation_efficiency(self,
        ...
        efficiency_score = (patterns_extracted / memories_processed) * \
            100 if memories_processed > 0 else 0
    ...
    async def _check_performance_alerts(self, operation_type: OperationType,
        ...
        max_time_threshold = self.alert_thresholds.get(
            "max_operation_time_ms", 5000)
        ...
        if profile.total_executions >= 10:  # Only check after sufficient operations
            min_success_rate = self.alert_thresholds.get(
                "min_success_rate", 95.0)
    ...
    async def generate_performance_report(self,
        ...
        try:
            ...
            total_operations = sum(
                profile.total_executions for profile in self.operation_profiles.values())
            total_errors = sum(
                profile.error_count for profile in self.operation_profiles.values())
            overall_success_rate = ((total_operations - total_errors) /
                                    total_operations * 100) if total_operations > 0 else 100
        ...
        for op_type in OperationType:
            ...
            if len(time_metrics) >= 2:
                ...
                sorted_metrics = sorted(
                    time_metrics, key=lambda m: m.timestamp)
                mid_point = len(sorted_metrics) // 2

                first_half_avg = statistics.mean(
                    [m.value for m in sorted_metrics[:mid_point]])
    ...
    async def _calculate_system_health_score(self) -> float:
        ...
        try:
            ...
            # Success rate score
            total_ops = sum(
                p.total_executions for p in self.operation_profiles.values())
            total_errors = sum(
                p.error_count for p in self.operation_profiles.values())
            success_rate = ((total_ops - total_errors) /
                            total_ops * 100) if total_ops > 0 else 100
            ...
            for op_type in OperationType:
                ...
                if profile.avg_time_ms > 0:
                    ...
                    if profile.min_time_ms > 0:
                        perf_score = max(
                            0, 100 - ((profile.avg_time_ms - profile.min_time_ms) / profile.min_time_ms * 50))
            ...
            alert_penalty = min(len(recent_alerts) * 5,
                                50)  # Max 50 point penalty
