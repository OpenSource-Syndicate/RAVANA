...
class SandboxEnvironment:
    ...
    async def setup(self):
        ...
        try:
            ...
            self.sandbox_dir = Path(tempfile.mkdtemp(prefix=f"snake_sandbox_{self.experiment_id}_"))
    ...
    async def execute_code(self, code: str, file_path: str, timeout: int = None) -> Tuple[bool, str, float]:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                sys.executable, '-c', f'exec(open("{target_file}").read())',
...
class CodeSafetyAnalyzer:
    ...
    def analyze_safety(self, code: str) -> Tuple[bool, List[str], float]:
                ...
                if isinstance(node, (ast.Import, ast.ImportFrom)):
                    ...
                    for module in module_names:
                        if module and any(dangerous in module for dangerous in self.dangerous_imports):
                            warnings.append(f"Dangerous import detected: {module}")
                            safety_score -= 0.2
                ...
                # Check for network operations
                    ...
                    if (isinstance(node.func, ast.Attribute) and 
                        ...
                        if any(net_op in func_name for net_op in ['socket.', 'urllib.', 'requests.']):
                            warnings.append(f"Network operation detected: {func_name}")
                            safety_score -= 0.1
        ...
        if len(code) > Config.SNAKE_MAX_FILE_SIZE:
            warnings.append(f"Code size ({len(code)}) exceeds maximum ({Config.SNAKE_MAX_FILE_SIZE})")
            safety_score -= 0.1
        ...
        for blacklisted_path in Config.SNAKE_BLACKLIST_PATHS:
            if blacklisted_path and blacklisted_path in code:
                warnings.append(f"Blacklisted path detected: {blacklisted_path}")
                safety_score -= 0.3
        
        safety_score = max(0.0, safety_score)
        is_safe = safety_score > 0.7 and len([w for w in warnings if 'Dangerous' in w]) == 0
...
class SnakeSafeExperimenter:
    ...
    async def run_experiment(self, experiment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Safety analysis
            is_safe, safety_warnings, safety_score = self.safety_analyzer.analyze_safety(improved_code)
            if not is_safe:
                logger.warning(f"Experiment {experiment_id} failed safety check: {safety_warnings}")
                return {
                    "success": False,
                    "safe": False,
            ...
            logger.info(f"Experiment {experiment_id} completed: success={result.success}, safe={result.safe}")
    ...
    async def _run_sandbox_experiment(self, experiment_id: str, file_path: str, 
        ...
        async with SandboxEnvironment(experiment_id) as sandbox:
            result = ExperimentResult(success=False, safe=True, test_passed=False)
            ...
            try:
                ...
                result.performance_impact = performance_metrics.get("avg_execution_time", 0.0)
                result.memory_usage = performance_metrics.get("memory_usage", 0.0)
                
                # Calculate impact score
                result.impact_score = self._calculate_impact_score(result, performance_metrics)
                
                logger.info(f"Sandbox experiment completed: success={result.success}, tests={result.test_passed}")
