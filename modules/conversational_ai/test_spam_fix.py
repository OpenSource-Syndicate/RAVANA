...
def test_spam_response_prevention():
    ...
    test_cases = [
        ...
        ("What is artificial intelligence?", {
         "dominant_mood": "Curious", "detected_interests": ["technology"]}),
        ("How does machine learning work?", {
         "dominant_mood": "Curious", "detected_interests": ["technology"]}),

        # Philosophical questions
        ("What is the meaning of consciousness?", {
         "dominant_mood": "Curious", "detected_interests": ["philosophy"]}),
        ("How do we define reality?", {
         "dominant_mood": "Curious", "detected_interests": ["philosophy"]}),

        # Casual conversation
        ("Hello, how are you?", {
         "dominant_mood": "Curious", "detected_interests": []}),
        ("I'm doing well today", {
         "dominant_mood": "Confident", "detected_interests": []}),

        # Edge cases that might trigger fallback responses
        # Empty message
        ("", {"dominant_mood": "Curious", "detected_interests": []}),
        # Minimal message
        ("...", {"dominant_mood": "Bored", "detected_interests": []}),
        # Just a question mark
        ("?", {"dominant_mood": "Confused", "detected_interests": []}),
    ]
    ...
    for i, (message, emotional_context) in enumerate(test_cases, 1):
        try:
            ...
            if not response or len(response.strip()) < 5:
                print(
                    f"✗ Test {i} FAILED: Response is empty or too short: '{response}'")
            ...
            if contains_spam:
                print(
                    f"✗ Test {i} FAILED: Response contains spam phrase '{found_spam_phrase}': {response}")
                all_tests_passed = False
            else:
                print(
                    f"✓ Test {i} PASSED: {response[:100]}{'...' if len(response) > 100 else ''}")
    ...
    for i, (message, emotional_context) in enumerate(fallback_cases, 1):
        try:
            ...
            if not response or len(response.strip()) < 10:
                print(
                    f"✗ Fallback Test {i} FAILED: Response is empty or too short: '{response}'")
                all_tests_passed = False
                continue
            ...
            is_generic = any(indicator.lower() in response.lower()
                             for indicator in generic_indicators)

            if is_generic:
                print(
                    f"✗ Fallback Test {i} FAILED: Response is too generic: {response}")
                all_tests_passed = False
            else:
