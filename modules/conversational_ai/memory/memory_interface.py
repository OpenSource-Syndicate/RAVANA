...
class SharedMemoryInterface:
        ...
        try:
            ...
            if os.path.exists(memory_path):
                try:
                    ...
                    logger.error(f"Error loading existing memory for {user_id}: {e}")
            ...
            if len(user_memory["conversations"]) > 100:  # Higher threshold for shared memory
    ...
    def _archive_old_conversations(self, user_id: str, user_memory: Dict[str, Any]):
        ...
        try:
            ...
            conversations_to_archive = user_memory["conversations"][:-50] if len(user_memory["conversations"]) > 50 else []
            ...
            if conversations_to_archive:
                ...
                logger.info(f"Archived {len(conversations_to_archive)} conversations for user {user_id}")
                
        except Exception as e:
            logger.error(f"Error archiving conversations for user {user_id}: {e}")
    ...
    def store_knowledge_fragment(self, user_id: str, fragment: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Error storing knowledge fragment for user {user_id}: {e}")
...
"""
            ...
            logger.error(f"Error summarizing chat history for user {user_id}: {e}")
            return None
...
"""
            ...
            topics_response = safe_call_llm(topic_prompt, timeout=20, retries=2)
    ...
    def get_chat_summaries(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error loading chat summaries for {user_id}: {e}")
                return []
        except Exception as e:
    ...
    def search_archived_conversations(self, user_id: str, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error searching archived conversations for user {user_id}: {e}")
            return []