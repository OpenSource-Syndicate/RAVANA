...
class ConversationalAI:
    ...
    async def start(self, standalone: bool = True):
        ...
        try:
            ...
            if standalone and bot_tasks:
                logger.info("Bots started in standalone mode, running indefinitely. Press Ctrl+C to stop.")
                logger.info(f"Number of bot tasks: {len(bot_tasks)}")
                logger.info(f"Shutdown event is set: {self._shutdown.is_set()}")
                try:
                ...
                logger.info("No bots available, running in standalone mode. Press Ctrl+C to stop.")
                try:
    ...
    def handle_task_from_user(self, user_id: str, task_description: str):
        ...
        """
        try:
            logger.info(f"Handling task from user {user_id}: {task_description}")
            ...
            logger.info(f"Task from user {user_id} sent to RAVANA for processing")
    ...
    def _synchronize_emotional_context(self, user_id: str, emotional_context: Dict[str, Any]):
        ...
        try:
            ...
            self.ravana_communicator.send_emotional_context_to_ravana(emotional_data)
        except Exception as e:
