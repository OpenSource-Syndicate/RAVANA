    ...
    async def migrate_from_chromadb(self, batch_size: int = 100):
        ...
        try:
            ...
            chroma_client = chromadb.Client(
                Settings(persist_directory=self.chroma_db_path, is_persistent=True)
            )
            ...
            try:
                ...
                logger.warning("ChromaDB collection 'memories' not found, skipping migration")
                return 0
            ...
            logger.info(f"Migration completed. Migrated {migrated_count} memories")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="RAVANA Multi-Modal Memory Database Setup")
    ...
    try:
        ...
        if not await migrator.check_pgvector_extension():
            ...
            logger.info("Installation guide: https://github.com/pgvector/pgvector")
            return 1
        ...
        logger.info("\nNext steps:")
        logger.info("1. Set POSTGRES_URL environment variable if not already set")
        logger.info("2. Install multi-modal dependencies: pip install -r requirements.txt")
