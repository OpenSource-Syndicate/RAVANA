...
class WhisperAudioProcessor:
    ...
    def __init__(self, model_size: str = "base", device: Optional[str] = None):
        ...
        if not WHISPER_AVAILABLE:
            raise ImportError(
                "Whisper dependencies not available. Install with: pip install openai-whisper librosa soundfile")
        ...
        logger.info(
            f"Initialized WhisperAudioProcessor with model_size={model_size}, device={self.device}")
    ...
    def _load_model(self):
        ...
        if self.model is None:
            try:
                ...
                logger.info(
                    f"Successfully loaded Whisper model on {self.device}")
    ...
    async def process_audio(self, audio_path: str, custom_prompt: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if audio_info["duration"] > self.max_duration:
                logger.warning(
                    f"Audio duration {audio_info['duration']:.1f}s exceeds maximum {self.max_duration}s")
            ...
            logger.info(
                f"Successfully processed audio: {audio_path} (duration: {audio_info['duration']:.1f}s)")
            return result
    ...
    async def _transcribe_audio(self, audio_path: str, custom_prompt: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Transcription completed. Language: {result.get('language', 'unknown')}")
            return result
            ...
            if "segments" in transcription_result and transcription_result["segments"]:
                ...
                for segment in transcription_result["segments"]:
                    if "avg_logprob" in segment:
                        ...
                        confidence = min(
                            1.0, max(0.0, np.exp(segment["avg_logprob"])))
