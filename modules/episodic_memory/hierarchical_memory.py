...
class HierarchicalMemoryStore:
    ...
    async def contextual_retrieval(self, query: str, context: Dict[str, Any], mood: Optional[Dict[str, float]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if mood:
                ...
                context_scores = self._apply_mood_modulation(
                    context_scores, mood_influence)
    ...
    async def _compute_context_relevance(self, query: str, context: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            for memory in all_memories:
                ...
                query_similarity = self._compute_text_similarity(
                    query, memory_text)
                ...
                for event in recent_events:
                    ...
                    event_similarity = self._compute_text_similarity(
                        event_context, memory_text)
                ...
                for result in search_results:
                    ...
                    result_similarity = self._compute_text_similarity(
                        result_text, memory_text)
    ...
    async def _compute_mood_influence(self, mood: Dict[str, float]) -> Dict[str, float]:
        ...
        # Find dominant mood
        dominant_mood = max(mood.items(), key=lambda x: x[1])[
                            0] if mood else ''
    ...
    def _apply_mood_modulation(self, scores: Dict[str, float], mood_influence: Dict[str, float]) -> Dict[str, float]:
        ...
        mood_factor = sum(mood_influence.values()) / \
                          len(mood_influence) if mood_influence else 0
        ...
        for memory_id in modulated_scores:
            ...
            modulated_scores[memory_id] = min(
                1.0, modulated_scores[memory_id])  # Cap at 1.0
    ...
    def _cleanup_cache(self):
        ...
        try:
            ...
            for key, value in self.context_cache.items():
                # 30 minutes
                if (current_time - value['timestamp']).total_seconds() > 1800:
