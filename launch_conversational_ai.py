...
async def verify_bots():
    ...
    try:
        ...
        # Test Discord
        discord_enabled = ai.config.get("platforms", {}).get("discord", {}).get("enabled", False)
        ...
        # Test Telegram
        telegram_enabled = ai.config.get("platforms", {}).get("telegram", {}).get("enabled", False)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Launch the Conversational AI Module")
    parser.add_argument("--verify-bots", action="store_true", help="Verify bot connectivity and exit")
